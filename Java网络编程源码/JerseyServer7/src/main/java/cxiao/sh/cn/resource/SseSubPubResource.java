...
@Component
@Path("chat")
public class SseSubPubResource  {
    private ConcurrentHashMap<String, List<EventOutput>> outputMap = new ConcurrentHashMap<>();
    @GET
    ...
    public EventOutput register(@PathParam("topic") String topic){
        ...
        if (!outputMap.containsKey(topic)){
            outputMap.put(topic,new ArrayList<>(Arrays.asList(output)));
            return output;
        }
    ...
    public void postStudent(Student stu, @PathParam("topic") String topic) throws Exception {
        ...
        if (outputMap.get(topic)==null){
            System.out.println("此主题" + topic + "未被订阅，无法发布！");
            return;
        }
        List<EventOutput> outputs = outputMap.get(topic);
        Iterator<EventOutput> iterator = outputs.iterator();
        while (iterator.hasNext()){
            EventOutput output = iterator.next();
            //客户端可能关闭事件通道，所以这里要清除已关闭的通道
            if (output.isClosed()) {
                iterator.remove();
            }else{
                output.write(event);
