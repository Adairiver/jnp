...
public class Client {
    ...
    private void download(String remoteFileName){
        ...
        if (response.getStatus()==Response.Status.NOT_FOUND.getStatusCode()){
            System.out.println("远程文件不存在：" + remoteFileName);
        }
        if (response.getStatus()==Response.Status.OK.getStatusCode()) {
            //下载至本地时文件名自动增加UUID以避免和已有文件重名
    ...
    //针对服务端 download0(.)函数的请求函数
    //仅作为对照，并不使用
    private void download0(String remoteFileName){
         Response response = target
                .path("/file/get")
                 .queryParam("name", remoteFileName)
                .request(MediaType.APPLICATION_OCTET_STREAM_TYPE)
                .get();
