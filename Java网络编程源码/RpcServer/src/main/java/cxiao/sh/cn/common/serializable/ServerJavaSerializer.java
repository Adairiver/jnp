...
public class ServerJavaSerializer implements IServerSerializer<Request, Response> {
    @Override
    public byte[] marshalling(Response response) throws Exception {
        return objSerializableToByteArray(response);
    }
    @Override
    public Request unmarshalling(byte[] data) throws Exception {
        return (Request)byteArrayToObjSerializable(data);
    }
